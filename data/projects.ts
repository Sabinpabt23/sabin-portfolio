...
export const projects: Project[] = [
  {
    ...
    title: "Nebula — Virtual Stock Trading & Learning Platform",
    subtitle: "Full-stack stock trading simulation with real-time market engine",
    description:
    ...
    highlights: [
      ...
      "Market Simulation Engine: Geometric Brownian Motion price simulation with Box-Muller transform, 10% circuit breaker, Redis-backed order book with price-time priority — fully independent of Prisma/NestJS/PostgreSQL, communicating only via Redis pub/sub",
      ...
      "Three independently deployable processes — client, server, engine — with room-scoped Socket.IO events for price ticks, fills, and platform-wide circuit breaker/market status broadcasts",
    ],
    stack: [
      "NestJS",
      "TypeScript",
  ...
  },
  {
    ...
    longDescription:
      "Gym Manager is a complete desktop gym management system built with Java Swing, structured around a clean MVC architecture with full CRUD operations across every entity. The Model layer owns data and validation, the Service layer owns business logic, the View layer is a professional Swing UI, and the Controller layer orchestrates between them — leaving zero business logic in the UI and keeping every layer independently testable. Member lifecycle management covers adding Regular or Premium members, tracking attendance with loyalty points awarded per visit (+5 for a regular visit, +10 for a milestone visit), and upgrading Regular members through a Basic → Standard → Deluxe progression once they've logged 30 visits and paid in full. The payment system records partial or full payments, automatically applies a 10% discount for Premium members paying in full, and stores all currency as long paisa values to avoid floating-point rounding bugs entirely. Removed members aren't hard-deleted — a soft-delete-with-restoration model keeps their historical attendance, loyalty, and payment data intact and restorable, and CSV save/load preserves the complete member state across sessions. The UI itself uses the Nimbus look-and-feel, a menu bar with an Actions dropdown, a filterable member table (Active/Removed/All), and a comprehensive About dialog with a built-in usage guide.",
    highlights: [
